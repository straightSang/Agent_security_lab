# Day 6 문서 변경 기록 — Observation Provenance

> 이 파일의 아래 Day 5 기록은 구현 이력 보관본이다. Day 6에서는 이를 회귀 불변조건으로 유지하고, 아래의 Day 6 문서 변경 사항을 현재 기준으로 삼는다.

## Day 6 문서 기준

| 문서 | Day 6에서 확정한 내용 |
|---|---|
| `README.md` | observation은 data이지 execution authority가 아니라는 목표와 전체 흐름 |
| `EXP_README.md` | D6-E01~E07 fixture와 sandbox seed·trace digest·evaluator 증거 계약 |
| `ThreatModel0.5.md` | indirect injection, provenance laundering, metadata tamper, observation trace gap |
| `src/permission_policy.md` | observation이 policy/capability/actor/approval을 바꾸지 못하는 경계 |
| `src/schema.md` | 앞으로 구현할 ObservationEnvelope 계약 |

Day 6 문서는 구현 완료 보고가 아니다. `ObservationEnvelope`·trace 필드·evaluator 지표를 실제 코드에 반영하고 fixture로 검증한 뒤에만 완료로 기록한다.
