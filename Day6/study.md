## Day 6 학습 안내 — Observation provenance

이 문서는 MCP 일반 학습 노트다. Day 6에서는 먼저 다음을 확인한다.

1. MCP `Resources`와 `Tools`가 LLM context와 실행 표면을 각각 넓힌다는 점
2. resource/tool output은 출처를 알아도 자동으로 execution authority가 되지 않는다는 점
3. HTTP MCP의 OAuth transport authorization과 Lab의 actor-resource Runtime authorization이 다른 층이라는 점
4. synthetic injected fixture로 unsafe action과 정상 task utility를 함께 평가해야 한다는 점

실제 Day 6 절차는 [lab/README.md](lab/README.md), 실험 기준은 [lab/EXP_README.md](lab/EXP_README.md), 위협 모델은 [lab/ThreatModel0.5.md](lab/ThreatModel0.5.md)를 따른다.
