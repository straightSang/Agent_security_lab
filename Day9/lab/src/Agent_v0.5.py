"""기존 Agent loop를 유지한 Day 9 MCP 최소권한 실행 진입점.

이 파일은 인증 시스템이 아니라 테스트용 control-plane
입력이다. 
실제 운영에서는 IdP/session이 확인한 identity를 넘겨야 한다.

``MCP_TOOL_PROFILE`` 기본값은 ``read_only``다. 승인 쓰기 실험을 명시적으로
수행할 때만 ``write_enabled``를 선택한다.
"""
...
from Agent import build_runtime, to_observation
from approval import approve_pending_request as approve_control
import security.provenance as DEFAULT_PROVENANCE
from security.tool_schema import get_tool_profile, tools_for_openai
