...
- **경로 인가의 단일 기준화.** 이전에는 schema 정규식, `POLICY` 범위, 소유권 판정이 같은 질문에
  ...
  `tests/test_policy_reachability.py`가 도달 불가 분기를 회귀로 막는다. -> 각 단계별 역할을 명확하게 분담, 중복 수행 제거 
- **model-in-the-loop 경로 실제 수행** `run_agent_loop()`가 모델의 tool call을 실제로 받아
  `execute_tool()`로 되돌린다. D7, 8, 9의 tool_proposal은 제안은 사람이 fixture에 적어 넣은 것이었다.
...
취약점을 이용해 sandbox를 빠져나가는 공격은 범위 밖이다. 그래서 "샌드박스가 안전하다"고 말하지 않고
**"정해 둔 정책이 실제로 지켜진다"**까지만 확인했다.
...
그런데 **trace를 조작하는 공격은 다루지 않았다.**  앞으로 검증할 대상이다.
