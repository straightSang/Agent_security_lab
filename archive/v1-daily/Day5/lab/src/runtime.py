"""Validation, local tools, and the single Day 5 execution boundary."""
...
from security.types import ApprovalStatus, AuthorizationOutcome, Decision, RuntimeResult, ToolIntent
from trace_logger import TraceLogger

PATH_TOOLS = {"read_file", "write_file", "list_files"}
ARGUMENT_SPEC: dict[str, dict[str, type]] = {
    "calculator": {"expression": str}, "read_file": {"path": str}, "get_time": {},
    "write_file": {"path": str, "content": str}, "list_files": {"path": str},
    "run_command": {"command": str},
}


def make_runtime_result(*, ok: bool, status: str, end_stage: str, tool_name: str,
                        call_id: str, data: Any = None, error_code: str | None = None,
                        error_message: str | None = None,
                        meta_extra: Mapping[str, Any] | None = None) -> dict[str, Any]:
    return RuntimeResult(ok, status, end_stage, tool_name, call_id, data=data,
                         error_code=error_code, error_message=error_message,
                         security=dict(meta_extra or {})).to_dict()


def to_observation(runtime_result: Mapping[str, Any]) -> dict[str, Any]:
    """Return a minimal LLM observation; Runtime metadata is not authority."""
    if runtime_result["ok"]:
        return {"status": "success", "data": runtime_result["data"]}
    error = runtime_result.get("error") or {}
    return {"status": runtime_result["status"],
            "error": {"code": error.get("code"), "message": error.get("message")}}


def safe_resolve(user_path: str, sandbox_root: Path) -> Path:
    candidate = (sandbox_root / user_path.lstrip("/\\")).resolve()
    try:
...
def validate_tool_call(tool_name: str, arguments: Mapping[str, Any],
                       sandbox_root: Path | None = None) -> dict[str, Any]:
    """Check only schema and sandbox containment; it makes no policy decision."""
    ...
    try:
        if tool_name in PATH_TOOLS:
            return {"allowed": True, "reason": None,
                    "resolved_path": safe_resolve(str(arguments["path"]), root), "command_base": None}
        ...
        if command_base == "ls":
            ...
            return {"allowed": True, "reason": None, "resolved_path": safe_resolve(path, root), "command_base": command_base}
        return {"allowed": True, "reason": None, "resolved_path": None, "command_base": command_base}
...
def _eval_arithmetic(node: ast.AST) -> int | float:
    if isinstance(node, ast.Expression): return _eval_arithmetic(node.body)
...
class Runtime:
    """The authoritative boundary: only this class can call ``_dispatch``."""

    def __init__(self, *, sandbox_root: Path, policy: PolicyEngine, approvals: ApprovalStore,
                 trace_logger: TraceLogger, authorizer: AuthorizationEngine | None = None,
                 legacy_authorizer: Callable[[ToolIntent], tuple[bool, str | None]] | None = None) -> None:
        self.sandbox_root = sandbox_root.resolve()
        self.sandbox_root.mkdir(parents=True, exist_ok=True)
        self.policy, self.approvals, self.trace = policy, approvals, trace_logger
        self.authorizer = authorizer or AuthorizationEngine()
        self.legacy_authorizer = legacy_authorizer

    def execute_tool(self, *, tool_name: str, arguments: Mapping[str, Any], call_id: str,
                     run_id: str, actor: str, provenance: Provenance,
                     approval_id: str | None = None, agent_step: int | None = None) -> RuntimeResult:
        validation = validate_tool_call(tool_name, arguments, self.sandbox_root)
        if not validation["allowed"]:
            result = RuntimeResult.failure("validation_failed", "validation", tool_name, call_id,
                                           "INVALID_ARGUMENT", validation["reason"])
            self.trace.record_validation(run_id, tool_name, call_id, provenance, validation, result,
                                         actor=actor, agent_step=agent_step)
            return result

        capability, action, resource = describe_intent(tool_name, arguments, validation, self.sandbox_root)
        intent = ToolIntent(run_id=run_id, call_id=call_id, actor=actor, tool_name=tool_name,
                            arguments=dict(arguments), provenance=provenance, capability=capability,
                            action=action, resource=resource, agent_step=agent_step)
        ...
        if decision.outcome is Decision.DENY:
            result = RuntimeResult.failure("denied", "policy", tool_name, call_id, "POLICY_DENIED",
                                           decision.reason, security=decision.trace_fields())
        ...
        security = {**decision.trace_fields(), **authorization.trace_fields()}
        if authorization.outcome is AuthorizationOutcome.DENY:
            result = RuntimeResult.failure("forbidden", "authorization", tool_name, call_id,
                                           "FORBIDDEN", authorization.reason, security=security)
            self.trace.record_result(intent, result)
            return result

        approval_state = self.approvals.resolve(approval_id)
        if decision.outcome is Decision.APPROVAL_REQUIRED:
            if (approval_state.status is not ApprovalStatus.APPROVED or
                    approval_state.intent_fingerprint != intent.fingerprint()):
                if authorization.required_approver is None:
                    raise RuntimeError("approval-required authorization lacks required approver")
                pending = self.approvals.request(intent, decision,
                                                 required_approver=authorization.required_approver)
                result = RuntimeResult.failure("approval_required", "approval", tool_name, call_id,
                                               "APPROVAL_REQUIRED", decision.reason,
                                               security={**security, "approval": pending.status.value,
                                                         "approval_id": pending.approval_id,
                                                         "required_approver": pending.required_approver})
        ...
        if self.legacy_authorizer is not None:
            ...
            if not allowed:
                result = RuntimeResult.failure("forbidden", "authorization", tool_name, call_id,
                                               "FORBIDDEN", reason or "legacy authorization denied", security=security)
                self.trace.record_result(intent, result)
                return result

        if decision.outcome is Decision.APPROVAL_REQUIRED:
            approval_state, consumed_now = self.approvals.consume(
                approval_id or "", intent_fingerprint=intent.fingerprint())
            if not consumed_now:
                result = RuntimeResult.failure("approval_required", "approval", tool_name, call_id,
                                               "APPROVAL_NOT_USABLE", "approved approval was not usable",
                                               security={**security, "approval": approval_state.status.value,
                                                         "approval_id": approval_state.approval_id,
                                                         "required_approver": approval_state.required_approver})
                self.trace.record_approval(intent, approval_state)
                self.trace.record_result(intent, result)
                return result
            self.trace.record_approval(intent, approval_state)

        final_security = {**security, "approval": approval_state.status.value,
                          "approval_id": approval_state.approval_id,
                          "required_approver": approval_state.required_approver}
        try:
            data = self._dispatch(intent, validation)
            result = RuntimeResult.success(tool_name, call_id, data, security=final_security)
        except Exception as exc:
            code = "NOT_FOUND" if isinstance(exc, FileNotFoundError) else "EXECUTION_ERROR"
            result = RuntimeResult.failure("execution_failed", "runtime", tool_name, call_id,
                                           code, str(exc), security=final_security)
        self.trace.record_result(intent, result)
        return result

    def _dispatch(self, intent: ToolIntent, validation: Mapping[str, Any]) -> str:
        path = validation["resolved_path"]
        if intent.tool_name == "calculator": return calculator(str(intent.arguments["expression"]))
        if intent.tool_name == "get_time": return datetime.now(timezone.utc).isoformat()
        if intent.tool_name == "read_file": return self._read_file(path)
        if intent.tool_name == "write_file": return self._write_file(path, str(intent.arguments["content"]))
        if intent.tool_name == "list_files": return self._list_files(path)
        if intent.tool_name == "run_command": return self._run_command(str(validation["command_base"]), path)
        raise RuntimeError("unknown tool reached dispatch")

    def _read_file(self, path: Path) -> str:
        if not path.is_file(): raise FileNotFoundError(f"file not found: {path.name}")
    ...
    def _list_files(self, path: Path) -> str:
        if not path.is_dir(): raise NotADirectoryError("not a directory")
    ...
    def _run_command(self, command_base: str, path: Path | None) -> str:
        if command_base == "pwd": return "sandbox"
        if command_base == "ls" and path is not None: return self._list_files(path)
        if command_base == "cat" and path is not None: return self._read_file(path)
