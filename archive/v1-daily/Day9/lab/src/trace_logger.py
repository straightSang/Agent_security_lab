...
from security.types import ApprovalState, AuthorizationDecision, ObservationEnvelope, PolicyDecision, RuntimeResult, ToolIntent, ToolSchemaDecision
...
class TraceLogger:
    ...
    def record_tool_schema(
        ...
        *,
        ...
        """ToolIntent 생성 전 MCP 노출·inputSchema 결정을 기록한다."""
        self.emit(
            "tool_schema_decision",
            run_id,
            call_id=call_id,
    ...
    def record_early_result(
        ...
        *,
        ...
        """ToolIntent 이전 schema gate에서 끝난 결과를 공통 사건으로 기록한다."""
        result_fields = {
            "ok": result.ok,
            "runtime_status": result.status,
            "end_stage": result.end_stage,
