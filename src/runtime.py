...
#         LLM tool proposal
#            |
#            +-- 0. MCP schema gate   노출 여부 + 인자 계약 (문자열만 본다)
...
#
#     순서 자체가 계약이다. 각 단계는 앞 단계가 통과했을 때만 호출되며, 실험은
...
#
# 이제 인자 계약 검사는 schema gate 하나만 한다. 이 단계는 '계약이 맞는가'가
...
#
#     여기는 계약을 검사하지 않는다. 경로를 정규화하려면 문자열 경로가 있어야
#     하는데 없으면 정규화 자체가 불가능하다. 그 최소 전제만 확인한다. 그래서
#     오류 코드도 '계약 위반'이 아니라 '이 단계가 쓸 수 없는 값'이라는 뜻의
...
class Runtime:
    ...
    #
    #         0. schema gate    -> tool_schema     미노출 도구, 인자 계약 위반
